...
- Added async Facebook Feed and Threads crossposting discovery and upload controls for photos, videos, albums, and clips, with regression and opt-in live coverage.

### Security

- Raised the Pillow minimum to 12.3.0 and refreshed test and documentation dependencies to patched versions.

## [1.12.6] - 2026-07-27

### Added

- Added `amount` and `last_media_pk` pagination controls to async `collection_medias_by_name()`, with regression and live coverage.
...
- Shut down the FBNS MQTT socket before closing it so active realtime reads unblock immediately during disconnect instead of waiting for the socket timeout.
- Synced the recorded upstream baseline to `instagrapi` 2.18.2.

## [1.12.1] - 2026-07-02

### Fixed

- Kept FBNS clients marked connected when a socket read times out without closing the MQTT connection, while still marking closed sockets and transport failures disconnected.
- Synced the recorded upstream baseline to `instagrapi` 2.18.1.

## [1.12.0] - 2026-06-30

### Added

- Exposed Instagram profile Threads badge fields on `User` as `show_text_post_app_badge` and `text_post_app_badge_label` for private v1 and public GraphQL user payloads.
- Synced the recorded upstream baseline to `instagrapi` 2.18.0.

## [1.11.2] - 2026-06-30

### Fixed

- Marked FBNS clients disconnected after MQTT transport send/read failures and made `fbns_disconnect()` clear broken socket state so long-running workers can reconnect cleanly.
- Synced the recorded upstream baseline to `instagrapi` 2.17.2.
...
- Documented the shared `USER_REPORT_REASON` Literal alias for `user_report(reason=...)`, including supported values and regression coverage for signatures and usage guides.
- Synced the recorded upstream baseline to `instagrapi` 2.16.24.

## [1.10.24] - 2026-06-20

### Changed

- Replaced the public Direct Notes `NoteAudience` magic-number alias with named enum values for `create_note()` and `create_music_note()`.
- Synced the recorded upstream baseline to `instagrapi` 2.16.23.

## [1.10.23] - 2026-06-20

### Fixed

- Documented the shared `NotificationSettingValue` option alias for low-level notification settings and added regression coverage for signatures and usage guides.
- Synced the recorded upstream baseline to `instagrapi` 2.16.22.
...
- Documented the shared `UserBlockSurface` Literal alias for `user_block(...)` and `user_unblock(...)`, including supported values and regression coverage for signatures and usage guides.
- Synced the recorded upstream baseline to `instagrapi` 2.16.20.

## [1.10.20] - 2026-06-20

### Fixed

- Documented the shared `DirectMediaType` Literal alias for Direct media helpers, including an option-types table and regression coverage for signatures and usage guides.
- Synced the recorded upstream baseline to `instagrapi` 2.16.19.

## [1.10.19] - 2026-06-20

### Fixed

- Documented the shared `MUSIC_PRODUCT` Literal alias for music browsing and Reel music upload helpers, including regression coverage for signatures and usage guides.
- Synced the recorded upstream baseline to `instagrapi` 2.16.18.
...
- Documented `direct_media_share(send_attribute=...)` with the public `SEND_ATTRIBUTE_MEDIA` Literal alias and added regression coverage for the signature.
- Synced the recorded upstream baseline to `instagrapi` 2.16.16.

## [1.10.16] - 2026-06-20

### Added

- Added `NotificationContentType` for account notification setting categories and documented notification helpers.
- Synced the recorded upstream baseline to `instagrapi` 2.16.15.
...
- Added a generated-docs-backed model reference for every public `aiograpi.types` Pydantic model.
- Synced the recorded upstream baseline to `instagrapi` 2.16.8.

## [1.10.7] - 2026-06-18

### Added

- Added async iterator helpers for paginated user follower/following, user media, and hashtag media lists.
- Synced the recorded upstream baseline to `instagrapi` 2.16.7.

## [1.10.6] - 2026-06-18

### Changed

- Mirrored `instagrapi` 2.16.6 CAA signup diagnostics so readable Instagram Bloks rejection messages are included when CAA account creation is refused without `created_user`.
- Synced the recorded upstream baseline to `instagrapi` 2.16.6.

## [1.10.5] - 2026-06-18

### Changed

- Mirrored `instagrapi` 2.16.5 media extraction so GraphQL inline comment previews are exposed on `Media.comments_preview` and `Media.hoisted_comments`.
- Synced the recorded upstream baseline to `instagrapi` 2.16.5.
...
- Mirrored `instagrapi` 2.16.2 challenge handling so `delta_acknowledge_approved` is treated as an acknowledgement step instead of raising `ChallengeUnknownStep`.
- Synced the recorded upstream baseline to `instagrapi` 2.16.2.

## [1.10.1] - 2026-06-18

### Changed

- Relaxed runtime dependency pins to conservative compatibility ranges while keeping the Android pydantic pin for Termux wheel compatibility.
- Added a minimum-runtime-dependencies CI job that installs the lower-bound runtime set, installs aiograpi, runs `pip check`, and imports `Client`.
- Synced the recorded upstream baseline to `instagrapi` 2.16.1.
...
- Added `Client.media_search(query, amount=...)` as the async mirror of instagrapi's private fbsearch media keyword search helper.

### Changed

- Synced the recorded upstream baseline to `instagrapi` 2.15.0.
...
- Fixed legacy `signup(...)` helper requests in logged-out/sessionless flows so the async pre-login guard does not raise `PreLoginRequired` before Instagram receives signup config, email, phone, age-check, or account-create requests.
...
- Added `coauthor_user_ids` to feed `photo_upload`, `video_upload`, and `album_upload` so callers can invite one or more Instagram collaborators without hand-building `extra_data`.

### Changed

- Synced the recorded upstream baseline to `instagrapi` 2.10.9.
...
- Closed generated Reel/IGTV thumbnail image handles during crop processing and rewrote thumbnails in binary mode, avoiding stale local file locks on Windows after video uploads.

### Changed

- Synced the recorded upstream baseline to `instagrapi` 2.10.7.

## [1.4.6] - 2026-06-14

### Fixed

- Added regression coverage for collection pagination responses that return the next cursor as `max_id` instead of `next_max_id`.

### Changed

- Synced the recorded upstream baseline to `instagrapi` 2.10.6.

## [1.4.5] - 2026-06-14

### Fixed

- Changed `fbsearch_suggested_profiles(...)` to return typed `UserShort` objects, matching its annotation and the sync client behavior.

### Changed

- Synced the recorded upstream baseline to `instagrapi` 2.10.5.
