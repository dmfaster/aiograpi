...
class LoginMixin(PreLoginFlowMixin, PostLoginFlowMixin):
    ...
    async def _login_with_bloks_two_factor(self, verification_code: str, login_json: Dict, exc: Exception) -> bool:
        ...
        await self.bloks_two_step_verification_entrypoint(context)
        await self.bloks_two_step_verification_method_picker(context)
        await self.bloks_two_step_verification_select_method(context, selected_method=challenge)
        if challenge == "backup_codes":
            await self.bloks_two_step_verification_enter_backup_code(context)
        code = self._normalize_backup_code(verification_code) if challenge == "backup_codes" else verification_code
        result = await self.bloks_two_step_verification_verify_code(
            context,
            code,
            challenge=challenge,
        )
    ...
    async def login(
        ...
        try:
            ...
            if self._looks_like_backup_code(verification_code) and self._extract_two_step_verification_context(
                ...
                logged = await self._login_with_bloks_two_factor(verification_code, two_factor_json, e)
            else:
                ...
                try:
                    ...
                    if message.strip().lower() == "invalid parameters":
                        logged = await self._login_with_bloks_two_factor(verification_code, two_factor_json, exc)
                    else:
                        raise
                else:
